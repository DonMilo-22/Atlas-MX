...
export default function MexicoMap({selectedState,onSelectState,zoom,setZoom,pan,setPan,showLabels,tourismFilter='todos'}){
 ...
 return <div className="map-viewport" ref={ref} onMouseDown={down} onMouseMove={move} onMouseUp={()=>drag.current=false} onMouseLeave={()=>{drag.current=false;setTip(null)}}>
   ...
   {ESTADOS.map(s=>{const count=getStateCategoryStats(s.cve)[tourismFilter]||0;const filtered=tourismFilter!=='todos';return paths[s.cve]&&<path key={s.cve} d={paths[s.cve]} className={`state-path ${selectedState?.cve===s.cve?'state-selected':''} ${filtered?(count>0?'state-filter-match':'state-filter-muted'):''}`} style={filtered?{'--category-strength':Math.min(1,.32+(count*.11))}:undefined} filter={selectedState?.cve===s.cve?'url(#glow)':undefined} onMouseEnter={e=>enter(s.cve,e)} onMouseMove={e=>enter(s.cve,e)} onMouseLeave={()=>setTip(null)} onClick={()=>onSelectState?.(getEstadoByCve(s.cve))} role="button" tabIndex="0" aria-label={`${s.nombre}${filtered?`, ${count} opciones de ${filterLabel}`:''}`} onKeyDown={e=>{if(e.key==='Enter'||e.key===' ')onSelectState?.(getEstadoByCve(s.cve))}}/>}) }
